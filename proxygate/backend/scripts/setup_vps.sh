...
echo ""
echo "Next steps:"
echo "1. Configure DNS: vpn.yourdomain.com -> $(curl -s ifconfig.me)"
echo "2. Get SSL certificate: certbot --nginx -d vpn.yourdomain.com"
echo "3. Create symlinks for strongSwan certificates:"
echo "   ln -sf /etc/letsencrypt/live/vpn.yourdomain.com/fullchain.pem /etc/swanctl/x509/fullchain.pem"
echo "   ln -sf /etc/letsencrypt/live/vpn.yourdomain.com/privkey.pem /etc/swanctl/private/privkey.pem"
echo "   ln -sf /etc/letsencrypt/live/vpn.yourdomain.com/chain.pem /etc/swanctl/x509ca/chain.pem"
echo "4. Deploy backend and frontend to /opt/proxygate/"
echo "5. Install Python dependencies: source /opt/proxygate/venv/bin/activate && pip install -r requirements.txt"
