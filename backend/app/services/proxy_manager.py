...
class ProxyManager:
    ...
    WHITELIST_PATH = "/etc/3proxy/allowed_ips.txt"
...
'''
        ...
        # IP access control is enforced by iptables on proxy ports (2053/8080/3128/1080)
        if unique_domains:
            all_domains_str = ",".join(unique_domains)
            config += f'''
# TLS proxy passthrough — IP filtering done by iptables on proxy ports
...
'''
    ...
    def write_whitelist(self, clients: List[ProxyClient]) -> None:
        """Write allowed IPs file for the TLS proxy whitelist check."""
        ips = set()
        for client in clients:
            if client.is_active:
                ips.update(client.allowed_ips)
        with open(self.WHITELIST_PATH, 'w') as f:
            for ip in sorted(ips):
                ip = ip.strip()
                if ip:
                    f.write(ip + "\n")
        logger.info(f"Whitelist written: {len(ips)} IPs to {self.WHITELIST_PATH}")

    @staticmethod
    def _reload_tls_proxy() -> None:
        """Send SIGHUP to TLS proxy to reload whitelist and certs."""
        import os as _os
        try:
            result = subprocess.run(
                ["pidof", "proxygate-tls-proxy"],
                capture_output=True, text=True
            )
            if result.returncode != 0:
                logger.debug("TLS proxy not running, skip SIGHUP")
                return
            for pid_str in result.stdout.strip().split():
                _os.kill(int(pid_str), signal.SIGHUP)
            logger.info("Sent SIGHUP to TLS proxy")
        except Exception as e:
            logger.warning(f"Failed to SIGHUP TLS proxy: {e}")
    ...
    def apply_changes(self, clients: List[ProxyClient]) -> bool:
        """Full cycle: generate config + restart + update iptables (IPv4 + IPv6) + TLS proxy whitelist."""
        self.write_config(clients)
        self.write_whitelist(clients)
        result = self.restart()
    ...
    def sync_iptables_whitelist(allowed_ips: set) -> None:
        ...
        Only whitelisted IPs can reach proxy ports (2053/8080/3128/1080/2096).
        Port 443 is NOT filtered — it serves the public web panel + XRay WS.
